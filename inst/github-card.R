...
hex <- image_read(here::here("man/figures/saffer-hex.png")) %>%
  image_scale("400x400")
...
gh_card <- image_canvas_ghcard("#ffffff") %>%
  ...
  image_annotate("saffer: ZAF Stuff", gravity = "West", location = "+100-30", color="#0d4448", size=50, font="Roboto Slab") %>%
  image_compose(gh_logo, gravity="West", offset = "+100+45") %>%
  image_annotate("datawookie/saffer", gravity="West", location="+160+45", size=50, font="Ubuntu Mono") %>%
